...
fetch('https://data.rennesmetropole.fr/api/explore/v2.1/catalog/datasets/decheteries_plateformes_vegetaux/exports/geojson?lang=fr&timezone=Europe%2FBerlin')
  ...
  .then(data => {
    ...
    map.on('load', () => {
      ...
      // limite communales Rennes Métropole
      map.addSource('sourcesRM', {
type: 'geojson',
...
"paint": {"line-color": "#03442a", "line-width": 0.4}
    ...
    function buildLocationList(stores) {
      for (const store of stores.features) {
        ...
        if (store.properties.ann_ouv) {
          details.innerHTML += `${"<br>Année d'ouverture : " + store.properties.ann_ouv}`;
        }
